...
class RETAIN_BI(DeepPredictor):
    def __init__(self, subject, ph, params, train, valid, test):
        ...
        self.model = self.RETAIN_Module(self.input_shape, self.params["n_features_emb"], self.params["n_hidden_rnn"],
                                        self.params["n_layers_rnn"], self.params["emb_dropout"], self.params["ctx_dropout"], self.params["reverse_time"])
    ...
    class RETAIN_Module(nn.Module):
        ...
        def __init__(self, n_in, n_features_emb, n_hidden_rnn, n_layers_rnn, emb_dropout, ctx_dropout, reverse_time=True):
            super().__init__()

            self.embeddings = nn.Linear(n_in, n_features_emb, bias=False)
            self.rnn_alpha = nn.LSTM(n_features_emb,n_hidden_rnn,n_layers_rnn,batch_first=True,bidirectional=True)
            self.rnn_beta = nn.LSTM(n_features_emb,n_hidden_rnn,n_layers_rnn,batch_first=True, bidirectional=True)
            ...
            self.alpha = nn.Sequential(
                nn.Linear(n_hidden_rnn + n_hidden_rnn,1,bias=True),
                nn.Softmax(dim=1)
                # Sparsemax(dim=1)
            )

            self.beta = nn.Sequential(
                nn.Linear(n_hidden_rnn + n_hidden_rnn, n_features_emb, bias=True),
                nn.Tanh()
            )
